...
### 3. Database Patterns
```typescript
// N+1 Prevention - ALWAYS use with
const posts = await db.query.posts.findMany({
  with: { author: true }  // NOT separate query
})
...
Every endpoint follows Auth->Validate->Ownership->Execute. See the API Route Template above.

OWASP essentials:
- IDOR: ownership check on every resource access
- Injection: parameterized queries only (Drizzle handles this)
